...
export function InteractiveBarChart() {
  ...
  const { data: chartData = [] } = useQuery({
    queryKey: ["dashboard", "recent-three-months"],
    queryFn: async () => {
      const response = await dashboardApi.getRecentThreeMonthsData();
  ...
  const total = React.useMemo(
    () => ({
      user: chartData.reduce((acc: number, curr: ChartData) => acc + curr.user, 0),
      generate: chartData.reduce((acc: number, curr: ChartData) => acc + curr.generate, 0),
    }),
    [chartData]
  );
