...
func (tq *TagQuery) QueryArtworks() *ArtworkQuery {
	query := (&ArtworkClient{config: tq.config}).Query()
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withArtworks: tq.withArtworks.Clone(),
...
func (tq *TagQuery) WithArtworks(opts ...func(*ArtworkQuery)) *TagQuery {
	query := (&ArtworkClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withArtworks = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withArtworks; query != nil {
		if err := tq.loadArtworks(ctx, query, nodes,
			func(n *Tag) { n.Edges.Artworks = []*Artwork{} },
			func(n *Tag, e *Artwork) { n.Edges.Artworks = append(n.Edges.Artworks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadArtworks(ctx context.Context, query *ArtworkQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Artwork)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.ArtworksTable)
		s.Join(joinT).On(s.C(artwork.FieldID), joinT.C(tag.ArtworksPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.ArtworksPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ArtworksPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "artworks" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
