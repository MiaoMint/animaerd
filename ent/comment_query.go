...
type CommentQuery struct {
	...
	withGeneratedArtwork *ArtworkQuery
	withFKs              bool
...
func (cq *CommentQuery) QueryLikes() *UserQuery {
	query := (&UserClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (cq *CommentQuery) QueryArtwork() *ArtworkQuery {
	query := (&ArtworkClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryGeneratedArtwork chains the current query on the "generated_artwork" edge.
func (cq *CommentQuery) QueryGeneratedArtwork() *ArtworkQuery {
	query := (&ArtworkClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withArtwork:          cq.withArtwork.Clone(),
		withGeneratedArtwork: cq.withGeneratedArtwork.Clone(),
...
func (cq *CommentQuery) WithLikes(opts ...func(*UserQuery)) *CommentQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) WithArtwork(opts ...func(*ArtworkQuery)) *CommentQuery {
	query := (&ArtworkClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withArtwork = query
	return cq
}

// WithGeneratedArtwork tells the query-builder to eager-load the nodes that are connected to
// the "generated_artwork" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommentQuery) WithGeneratedArtwork(opts ...func(*ArtworkQuery)) *CommentQuery {
	query := (&ArtworkClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withGeneratedArtwork = query
	return cq
}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	var (
		...
		loadedTypes = [6]bool{
			...
			cq.withLikes != nil,
			cq.withArtwork != nil,
			cq.withGeneratedArtwork != nil,
		}
	)
	if cq.withParent != nil || cq.withAuthor != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withLikes; query != nil {
		if err := cq.loadLikes(ctx, query, nodes,
			func(n *Comment) { n.Edges.Likes = []*User{} },
			func(n *Comment, e *User) { n.Edges.Likes = append(n.Edges.Likes, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withArtwork; query != nil {
		if err := cq.loadArtwork(ctx, query, nodes,
			func(n *Comment) { n.Edges.Artwork = []*Artwork{} },
			func(n *Comment, e *Artwork) { n.Edges.Artwork = append(n.Edges.Artwork, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withGeneratedArtwork; query != nil {
		if err := cq.loadGeneratedArtwork(ctx, query, nodes, nil,
			func(n *Comment, e *Artwork) { n.Edges.GeneratedArtwork = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadLikes(ctx context.Context, query *UserQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(comment.LikesPrimaryKey[1]))
...
func (cq *CommentQuery) loadArtwork(ctx context.Context, query *ArtworkQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Artwork)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Comment)
	nids := make(map[int]map[*Comment]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(comment.ArtworkTable)
		s.Join(joinT).On(s.C(artwork.FieldID), joinT.C(comment.ArtworkPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(comment.ArtworkPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(comment.ArtworkPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "artwork" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (cq *CommentQuery) loadGeneratedArtwork(ctx context.Context, query *ArtworkQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Artwork)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Comment)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Artwork(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(comment.GeneratedArtworkColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.comment_generated_artwork
		if fk == nil {
			return fmt.Errorf(`foreign-key "comment_generated_artwork" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "comment_generated_artwork" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
