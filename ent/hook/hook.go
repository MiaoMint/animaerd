...
type ArtworkFunc func(context.Context, *ent.ArtworkMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ArtworkFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ArtworkMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f ComfyUINodeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ComfyUINodeMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ComfyUINodeMutation", m)
}
