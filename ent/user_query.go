...
// QueryLikedArtworks chains the current query on the "liked_artworks" edge.
func (uq *UserQuery) QueryLikedArtworks() *ArtworkQuery {
	query := (&ArtworkClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
// WithLikedArtworks tells the query-builder to eager-load the nodes that are connected to
// the "liked_artworks" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithLikedArtworks(opts ...func(*ArtworkQuery)) *UserQuery {
	query := (&ArtworkClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withLikedArtworks = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withLikedArtworks; query != nil {
		if err := uq.loadLikedArtworks(ctx, query, nodes,
			func(n *User) { n.Edges.LikedArtworks = []*Artwork{} },
			func(n *User, e *Artwork) { n.Edges.LikedArtworks = append(n.Edges.LikedArtworks, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadArtworks(ctx context.Context, query *ArtworkQuery, nodes []*User, init func(*User), assign func(*User, *Artwork)) error {
	...
	for _, n := range neighbors {
		fk := n.user_artworks
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_artworks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_artworks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadLikedArtworks(ctx context.Context, query *ArtworkQuery, nodes []*User, init func(*User), assign func(*User, *Artwork)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.LikedArtworksTable)
		s.Join(joinT).On(s.C(artwork.FieldID), joinT.C(user.LikedArtworksPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.LikedArtworksPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.LikedArtworksPrimaryKey[0]))
