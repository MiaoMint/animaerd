...
func (mq *MediaQuery) QueryArtworks() *ArtworkQuery {
	query := (&ArtworkClient{config: mq.config}).Query()
...
func (mq *MediaQuery) Clone() *MediaQuery {
	...
	return &MediaQuery{
		...
		withArtworks: mq.withArtworks.Clone(),
...
func (mq *MediaQuery) WithArtworks(opts ...func(*ArtworkQuery)) *MediaQuery {
	query := (&ArtworkClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withArtworks = query
	return mq
}
...
func (mq *MediaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Media, error) {
	...
	if query := mq.withArtworks; query != nil {
		if err := mq.loadArtworks(ctx, query, nodes,
			func(n *Media) { n.Edges.Artworks = []*Artwork{} },
			func(n *Media, e *Artwork) { n.Edges.Artworks = append(n.Edges.Artworks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MediaQuery) loadArtworks(ctx context.Context, query *ArtworkQuery, nodes []*Media, init func(*Media), assign func(*Media, *Artwork)) error {
	...
	query.Where(predicate.Artwork(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(media.ArtworksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.media_artworks
		if fk == nil {
			return fmt.Errorf(`foreign-key "media_artworks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "media_artworks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
